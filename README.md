...
- `/` — landing page, `/rekisteroidy` — create an account (verification email → `/vahvista-sahkoposti`, resend with a 60 s cooldown), `/vahvista?token=` — the link from the email (verified / expired / used), `/login` — sign in, `/dashboard` — your watch areas on a map
...
```
src/
  app/(frontend)/         root document + error boundaries; (auth)/ login, rekisteroidy (+ register action), vahvista-sahkoposti (+ resend action, button), vahvista; (marketing)/ landing, 404, /huolto, /liikaa-pyyntoja, /virhe (test hook); (app)/dashboard
