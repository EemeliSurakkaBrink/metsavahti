...
### MV-070 Jobs infrastructure
**Goal:** Configure Payload Jobs queue; `job_runs` writer helper (`withJobRun(task, fn)` recording status/stats/error); `POST /api/jobs/run` (Bearer `CRON_SECRET`, enqueues `pipeline`, returns runId); `pnpm jobs:run` script executing queue once; admin "Run pipeline now" button (admin only). **Tests:** integration: unauthenticated 401; run creates `job_runs` row. **Depends on:** MV-035, MV-010
...
### MV-073 Alert email templates
**Goal:** `AlertImmediate` (one watch area, N alerts) and `AlertDigest` (grouped by area) React Email templates with Finnish copy, cutting-type explanation lines, distance formatting ("noin 350 m tontistasi"), CTA to `/ilmoitukset/[id]`, unsubscribe link (signed via MV-055 token), settings link, attribution; optional static-map `<img>` when `ENABLE_MAP_IMAGES`. **Tests:** unit snapshots; assert required links present; HTML < 100 KB. **Depends on:** MV-040, MV-028, MV-055
...
### MV-075 pipeline orchestration task
**Goal:** `pipeline` runs fetch → match → notify sequentially with shared `runId`; skips notify if match failed; overall `job_runs` summary; guard against overlapping runs (advisory lock). **Tests:** integration end-to-end with fixtures: area + fake WFS → email in Mailpit; second run → no email; concurrent trigger → second returns 409. **Depends on:** MV-071, MV-072, MV-074

### MV-076 cleanup task
**Goal:** Per `01 §4.4` using `retention.ts`; scheduled weekly via `POST /api/jobs/run?task=cleanup`. **Tests:** integration: expired declarations removed, referenced ones kept. **Depends on:** MV-070, MV-033

### MV-077 Static map image endpoint (flagged)
**Goal:** `GET /api/alerts/[id]/map-image.png` (owner session or signed token): SVG composed from watch circle + polygon (3067 → local pixel space, no tiles) rendered with `@resvg/resvg-js`, cached 7 d. Behind `ENABLE_MAP_IMAGES`. **Tests:** integration: returns PNG, 403 for non-owner, token OK. **Depends on:** MV-034, MV-047

### MV-078 Health endpoint pipeline status
**Goal:** Extend `/api/health` with last pipeline run (status, finishedAt, stats), `stale: true` if > 14 h old; `?deep=1` performs WFS GetCapabilities. **Tests:** integration. **Depends on:** MV-070
