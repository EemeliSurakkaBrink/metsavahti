...
| `lib/errors.ts`                                                              | Typed errors (`AppError` → `NotFound`, `Forbidden`, `RateLimited`, `ExternalServiceError`) and the one error → response mapping | Server code throws `AppError` subclasses; `actionResult()` wraps Server Actions (`{ ok, data                                                                | error }`), `toErrorResponse()`wraps Route Handlers. Unknown errors are logged and reach the client only as a generic`internal` message. |
